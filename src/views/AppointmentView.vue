...
<template>
  <div class="border-solid border-4 border-primary rounded-xl p-5 mt-10 mb-10 bg-secondary flex flex-col w-3/4 m-auto"
    ...
    <template v-if="availabilityStore.availabilities.length === 0">
      <h2 class="text-tertiary text-lg text-center mt-5">Ce docteur n'a pas de rendez-vous a proposer, consultez un
        autre docteur.</h2>
    </template>
    <template v-else>
      <h2 class="text-tertiary text-2xl self-center mt-10">Diponibilités :</h2>
      <vue-inline-calendar @update:selected-date="selectedDate = $event; selectedTime = null"
        :spec-min-date="new Date()" locale="fr-FR" :showYear="false" />
      <div class="grid grid-cols-3 gap-4">
        <template v-if="hasAvailableTimeSlots">
          <template v-for="creneau in availabilityStore.availabilities" :key="creneau.availabilityId">
            <div v-if="creneau.startDate == formattedDate" class="col-span-1">
              <input type="radio" name="option" :id="creneau.availabilityId" :value="creneau.availabilityId"
                class="peer hidden" v-model="selectedTime" />
              <label :for="creneau.availabilityId"
                class="bg-tertiary transition-colors duration-300 text-white font-semibold block border-solid border-5 border-tertiary cursor-pointer select-none rounded-xl p-2 text-center peer-checked:bg-primary peer-checked:font-bold peer-checked:text-white">
        ...
        <div v-else class="col-span-3 flex justify-center items-center text-2xl font-bold text-primary">
          <div class="text-center">
            <p>Oups, il n'y a plus de créneau disponible pour cette date.</p>
            <p>La prochaine date disponible est le <span class="text-tertiary">{{ nextAvailableDate() }}</span>.</p>
          </div>
        </div>
      </div>
      <div v-if="selectedTime" class="flex flex-col">
        ...
        <div class="w-3/4 self-center">
          ...
          <textarea id="message" class="border-solid border-2 border-primary rounded-xl p-2 w-full h-32 resize-none"
            v-model="rdvDescription"></textarea>
          <h3 class="text-quartiary text-lg font-semibold text-quartiary rounded-full bg-primary w-fit p-2">Prix de la
            consultation : {{ doctorStore.doctor.prixConsultation }} €</h3>
        </div>
        <button
          class="bg-tertiary border-none rounded-lg px-3 py-1 text-white font-bold transition-colors duration-300 cursor-pointer hover:bg-primary w-fit self-end"
          @click="validateAppointement">Valider le Rendez-vous</button>
      </div>
    </template>
  </div>
