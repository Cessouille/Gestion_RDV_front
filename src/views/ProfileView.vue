...
async function cancelAppointement(id) {
  if (window.confirm("Êtes vous sûr de vouloir annuler ce rendez-vous ?")) {
    try {
      await appointementStore.deleteAppointement(id);
      ...
      $toast.error('Erreur lors de la suppression du truc.', {
        position: 'top',
        duration: 3000,
      });
    }
    ...
    <div class="border-solid border-4 border-primary rounded-xl p-5 mt-10 mb-10 bg-secondary flex flex-col w-2/4 m-auto">
        ...
        <template v-else>
          ...
          <div class="border-solid border-4 border-primary mb-5 rounded-2xl">
            ...
            <button class="bg-tertiary border-none rounded-lg px-3 py-1 m-3 text-white font-bold transition-colors duration-300 cursor-pointer hover:bg-primary w-fit self-end" @click="cancelAppointement(appointement.rendezVousId)">Annuler le Rendez-vous</button>
            </div>
          </div>
